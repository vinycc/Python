# Importar módulos do sqlalchemy
import sqlalchemy
...
if func is not None:                            # Se não existir, retorna None
    ...
    print("O funcionario nao esta cadastrado")


# Busca utilizando filtros
# salario maior que 1500
print('-'*30)
d = session.query(Funcionario).filter(Funcionario.salario > 1500).order_by(Funcionario.nome)
for i in d:
    print(i.id, i.nome, i.idade, i.salario)
...
print('-'*30)
d = session.query(Funcionario).filter(Funcionario.idade == 22).filter(Funcionario.salario > 1500).filter(Funcionario.nome.like('%inho%')).all()
for i in d:
    print(i.id, i.nome, i.idade, i.salario)
...
print('-'*30)
d = session.query(Funcionario).filter(Funcionario.idade == 22).filter(Funcionario.nome.like('%inho%')).first()
