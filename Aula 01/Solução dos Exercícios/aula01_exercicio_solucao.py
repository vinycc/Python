...
def soma_divisores(n):
    a = 1
    soma = 0                    # variavel somadora começa em zero
    while a <= n:               # a = [1,2,3,4,5,6,....., n]
        if n % a == 0:          # verifica se o valor é um divisor de n
            soma = soma + a     # realiza o somatório
        a = a + 1
    return soma

def soma_divisores_for(n):
    soma = 0
    for a in range(1, n+1):     # a = [1,2,3,4,5,...., n]
        if n % a == 0:
            soma = soma + a
    return soma
